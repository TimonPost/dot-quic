...
pub(super) fn next_thread_id() -> usize {
    GLOBAL_ID.fetch_add(1, Ordering::SeqCst)
}

pub(super) fn get_thread_id() -> usize {
    THREAD_ID.with(|x| *x)
}
...
fn next_value_id() -> usize {
    VALUE_ID.with(|x| {
        // The value never has overlapping mutable aliases
        unsafe {
            let x = x.get();
            let next = *x;
            *x += 1;
...
impl<T: 'static> Drop for DeferredCleanup<T> {
    fn drop(&mut self) {
        if mem::needs_drop::<T>() {
            if self.is_valid() {
                // The value exists on the current thread
                unsafe {
...
impl<T: 'static> DeferredCleanup<T> {
    ...
    fn with_value<F: FnOnce(&UnsafeCell<Box<T>>) -> R, R>(&self, f: F) -> R {
        ...
        REGISTRY.with(|registry| {
            // There are no active mutable references
            unsafe {
    ...
    // The value must originate on the current thread
...
unsafe impl<T: 'static> Send for DeferredCleanup<T> {}

impl<T: 'static> Deref for DeferredCleanup<T> {
    type Target = T;

    fn deref(&self) -> &T {
        // The borrow of self protects the inner value
